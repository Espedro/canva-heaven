...
router.post("/signup", async (req, res) => {
  try {
    ...
    //     // Another check ---> validate if email address is in the proper format (checks if theres word@word.com )
    ...
    //     // Yet another check ---> verify if the password provided is in the correct format, use  regex to validate the format.
    const passwordRegex =
      /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$ %^&*-]).{6,}$/
